...
function extractBracedVariables(text: string): string[] {
...
function pathToRegex(pathTemplate: string) {
  // Escape all RegExp special characters except for the placeholders
  let escapedPath = pathTemplate.replace(
    /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,
    "\\$&",
  );
 ...
 */
export const makeOpenapiPathRouter = (openapi: OpenapiDocument) => {
  ...
  const matcher = (pathWithoutQuery: string) => {
    ...
    const context = mergeObjectsArray(
      match.variables.map((name, index) => ({
